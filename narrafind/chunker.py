"""Video chunking logic using ffmpeg."""
...
def chunk_video(
    ...
    tmp_dir = tempfile.mkdtemp(prefix="narrafind_")
    ...
    if duration <= chunk_duration:
        ...
        subprocess.run(
            [ffmpeg_exe, "-y", "-ss", "0", "-i", video_path,
             "-t", str(duration), "-c", "copy", chunk_path],
            capture_output=True,
            check=True,
        )
        return [
    ...
    while start < duration:
        ...
        subprocess.run(
            [ffmpeg_exe, "-y", "-ss", str(start), "-i", video_path,
             "-t", str(t), "-c", "copy", chunk_path],
            capture_output=True,
            check=True,
        )
...
def extract_audio(video_path: str, output_path: str | None = None) -> str:
    """Extract audio track from a video file as WAV for Whisper.
    ...
    if output_path is None:
        fd, output_path = tempfile.mkstemp(suffix=".wav", prefix="narrafind_audio_")
        os.close(fd)
...
def extract_audio_chunk(
    ...
    """Extract a specific audio segment from a video file as WAV.
...
def is_still_frame_chunk(
    ...
    try:
        ...
        tmp_dir = tempfile.mkdtemp(prefix="narrafind_still_")
